...
setMethod("print", "pixel.fig",
       definition=function(x, ...){
       		...
       		p1 <- ggplot(toprint.m, aes(X2, X1)) + geom_tile(aes(fill = value), colour="black") + scale_fill_manual(values=getColors(x@my.color.palette))+
labs(x = "",y = "")+theme(legend.position = "none",axis.ticks = element_blank(),axis.text.x=element_blank(),axis.text.y=element_blank())
